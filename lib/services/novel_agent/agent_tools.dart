...
class AgentTools {
      ...
      'parameters': {
        ...
        'properties': {
          ...
          'instruction': {
            'type': 'string',
            'description':
                '创作要求（自然语言，尽量精简）。一两句话说明本章要写什么即可，'
                '例如「主角在酒馆遇到神秘老人，获得关键线索」「写一段紧张的追逐戏」。'
                '不要把具体台词、对话展开、逐段情节大纲或成文内容写进指令——'
                '情节细节和对话由生成模型自由发挥，指令写得过细会束缚创作。',
          },
          'title': {
            'type': 'string',
  ...
  static const _rewriteChapter = {
    ...
    'function': {
      ...
      'description':
          'AI 重写指定章节的整章正文。\n'
          '本工具不会直接覆盖——它会读取章节原文，结合「修改要求」、「上一章正文'
          '（自动注入，最多末尾 5000 字，用于保持与前文细节一致）」、「人物卡」和「写作技巧」'
      ...
      'parameters': {
        ...
        'properties': {
          ...
          'rewriteInstruction': {
            'type': 'string',
            'description':
                '修改要求（自然语言，尽量精简）。一两句话说明希望本章改成什么样即可，'
      ...
      'parameters': {
        ...
        'properties': {
          ...
          'aspect_ratio': {
            'type': 'string',
            'description':
                '画面比例（"宽:高"，如 "1:1"、"3:4"、"4:3"、"9:16"、"16:9"），'
                '默认 1:1。仅 SDXL 类模型生效（竖版人物用 3:4 或 9:16，'
                '风景/横幅用 16:9）；SD1.5 模型忽略此参数恒为 1:1。',
          },
        },
        'required': ['prompt'],
      },
